...
class _PersonalProfilePageState extends ConsumerState<PersonalProfilePage>
  ...
  Future<void> _saveChanges() async {
    ...
    for (var i = 0; i < _emailControllers.length; i++) {
      ...
      final existingId = i < (_profile?.emails.length ?? 0)
          ? _profile!.emails[i].id
          : (i < (_profile?.communications.length ?? 0)
                ? _profile!.communications[i].emailId
                : null);
      emails.add(
        UserProfileApiClient.emailWrite(
          id: existingId,
          email: email,
    ...
    for (var i = 0; i < _phoneControllers.length; i++) {
      ...
      final phone = PhoneNumberUtils.formatFull(country, national);
      final existingId = i < (_profile?.phones.length ?? 0)
          ? _profile!.phones[i].id
          : (i < (_profile?.communications.length ?? 0)
                ? _profile!.communications[i].phoneId
                : null);
      phones.add(
        UserProfileApiClient.phoneWrite(
          id: existingId,
          phone: phone,
    ...
    final existingAddresses = _profile?.addresses ?? const <UserAddressModel>[];
    final addresses = <Map<String, dynamic>>[
      UserProfileApiClient.addressWrite(
        id: existingAddresses.isNotEmpty ? existingAddresses.first.id : null,
        address1: _addr1Controller.text.trim(),
        address2: _addr2Controller.text.trim(),
        country: existingAddresses.isNotEmpty
            ? existingAddresses.first.country
            : (_profile?.country ?? ''),
    ...
    if (_showExtraAddress) {
      addresses.add(
        UserProfileApiClient.addressWrite(
          id: existingAddresses.length > 1 ? existingAddresses[1].id : null,
          address1: _extraAddr1.text.trim(),
          address2: _extraAddr2.text.trim(),
          country: existingAddresses.length > 1
              ? existingAddresses[1].country
              : '',
          city: _extraCity.text.trim(),
          state: _extraState.text.trim(),
          pincode: _extraZip.text.trim(),
          isPrimary: false,
        ),
      );
    }

    // Lock immediately so app-bar check + Save button cannot fire twice.
    ...
    try {
      ...
      final updated = await api.replaceProfile(
        ...
        firstName: firstName,
        lastName: lastName,
        dateOfBirth: _dateOfBirthForApi(_dobController.text),
        gender: gender,
        emails: emails,
        phones: phones,
        addresses: addresses,
  ...
  /// API expects `YYYY-MM-DD`; UI may show `MM/DD/YYYY` after the date picker.
  static String _dateOfBirthForApi(String raw) {
    final t = raw.trim();
    if (t.isEmpty) return '';
    final iso = RegExp(r'^(\d{4})-(\d{2})-(\d{2})');
    final isoMatch = iso.firstMatch(t);
    if (isoMatch != null) {
      return '${isoMatch.group(1)}-${isoMatch.group(2)}-${isoMatch.group(3)}';
    }
    final slash = RegExp(r'^(\d{1,2})/(\d{1,2})/(\d{4})$');
    final slashMatch = slash.firstMatch(t);
    if (slashMatch != null) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          ...
          'Bottom sheet: no top menu icon; use More (â‹¯) on the bar to open Sites, Contacts, Groups, Products, and Quotations.',
