...
class AddJobPage extends ConsumerStatefulWidget {
  ...
  static const editStandaloneName = 'edit-service-job';
  ...
  static String standaloneEditPathFor(String jobId) =>
      '/jobs/${Uri.encodeComponent(jobId)}/edit';
...
class _AddJobPageState extends ConsumerState<AddJobPage> {
  ...
  Future<void> _loadServiceForms({bool preserveSelection = false}) async {
    final previousSelection = preserveSelection
        ? List<NamedIdOption>.from(_selectedForms)
        : const <NamedIdOption>[];
    ...
    try {
      final formRows = await ref.read(formsApiClientProvider).fetchForms();
      final options = formRows.toActivePickerOptions();
      if (!mounted) return;
      setState(() {
        _formOptions = options;
  ...
  void _onProjectSelected(ProjectOption? project) {
    ...
    if (projectId != null) {
      ...
      unawaited(_loadServiceForms(preserveSelection: true));
    }
  }
  ...
  Map<String, dynamic> _buildCreateJobPayload() {
    ...
    return JobWritePayload.build(
      ...
      jobCategory: _effectiveProjectId.isEmpty
          ? JobApiCategory.service
          : JobApiCategory.project,
      schedule: _sourceJob?.scheduleDetail,
      existingJobRaw: _sourceJob?.raw,
