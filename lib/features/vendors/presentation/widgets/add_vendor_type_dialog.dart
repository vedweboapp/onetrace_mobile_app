...
/// `POST /vendor-type/` — name, background color, text color.
Future<VendorTypeOption?> showAddVendorTypeDialog({
  required BuildContext context,
}) {
  return showDialog<VendorTypeOption>(
    context: context,
    barrierDismissible: false,
    builder: (context) => const _AddVendorTypeDialog(),
  );
}

class _AddVendorTypeDialog extends ConsumerStatefulWidget {
  const _AddVendorTypeDialog();

  @override
  ConsumerState<_AddVendorTypeDialog> createState() =>
      _AddVendorTypeDialogState();
}

class _AddVendorTypeDialogState extends ConsumerState<_AddVendorTypeDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      final created = await ref.read(vendorsApiClientProvider).createVendorType(
            VendorTypeWritePayload.build(
      ...
      setState(() {
        ...
        _errorMessage = ApiResponseMessage.fromAnyError(
          e,
          genericFallback: 'Failed to create vendor type',
        );
      });
    }
  }
              ...
              children: [
                ...
                _colorField(
                  ...
                  onColorChanged: (value) {
                    setState(() {
                      _bgColor = value;
                      _bgHexController.text = toHexRgb(value);
                    });
                  },
                ),
                const SizedBox(height: 16),
                _colorField(
                  ...
                  onColorChanged: (value) {
                    setState(() {
                      _textColor = value;
                      _textHexController.text = toHexRgb(value);
                    });
                  },
                ),
